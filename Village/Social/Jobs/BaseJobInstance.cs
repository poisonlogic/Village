...
{
    public abstract class BaseJobInstance<TDef> : BaseDimRunnableInstance<TDef> where TDef : JobDef
    {
        ...
        public BaseJobInstance(IDimProvider<TDef> provider, IDimManager<TDef> manager, TDef def) : base(provider, manager, def)
        {
        }

        public virtual bool CanAcceptWorker(IJobWorker<TDef> worker) => CanAcceptUser(worker);
        public override bool CanAcceptUser(IDimUser<TDef> user)
        {
            return base.CanAcceptUser(user);
        }
        
        public virtual bool TryAddWorker(IJobWorker<TDef> worker) => TryAddUser(worker);
        public override bool TryAddUser(IDimUser<TDef> user)
        {
